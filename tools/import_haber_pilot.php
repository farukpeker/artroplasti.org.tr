...
foreach ($pilotFiles as $fileName) {
    $htmlPath = $htmlDir . '/' . $fileName;

    if (!file_exists($htmlPath)) {
        $results[] = "SKIP {$fileName} (html bulunamadı)";
        continue;
    }
    ...
    if ($imageNode) {
        ...
        if (file_exists($localImagePath)) {
            ...
            if (!is_wp_error($postStubId) && $postStubId > 0) {
                $attachmentId = import_local_image_as_attachment($localImagePath, $postStubId);
                if ($attachmentId > 0) {
                    set_post_thumbnail($postStubId, $attachmentId);
                }

                update_post_meta($postStubId, '_import_source_html', $fileName);
                $results[] = "OK {$fileName} -> PostID {$postStubId}" . ($attachmentId ? " (thumb {$attachmentId})" : " (thumb yok)");
                continue;
            }
    ...
    $results[] = "OK {$fileName} -> PostID {$postId} (thumb yok)";
}

echo "Pilot import tamamlandı:\n";
